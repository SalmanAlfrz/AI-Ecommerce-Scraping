...
const ITEMS_PER_BATCH_PAGE = 10;
const ITEMS_PER_BATCH_DESC = 5;
const MAX_PAGES = 3;
  ...
  while (true) {
    ...
    for (let i = 0; i < rawHtmlItems.length; i += ITEMS_PER_BATCH_PAGE) {
      ...
      try {
        ...
        if (i + ITEMS_PER_BATCH_PAGE < rawHtmlItems.length) await delay(1000);
      } catch {
    ...
    for (let j = 0; j < filtered.length; j += ITEMS_PER_BATCH_DESC) {
      ...
      if (j + ITEMS_PER_BATCH_DESC < filtered.length) await delay(1000);
    }
